<nav x-data="{ scrolled: false, showDropdown: false }" 
  ...
  <div class="max-w-screen-xl flex flex-wrap items-center justify-end sm:justify-between mx-auto p-4">
    ...
    <div class="flex md:order-2 space-x-3 md:space-x-0 rtl:space-x-reverse">
      ...
      <?php else:?>
        <a href="/auth/login" type="button"  class="text-white hover:text-white border border-white hover:bg-black-200 focus:ring-4 focus:outline-none focus:ring-[#3FC43B] font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2 mb-2">Login</a>
        <a href="/auth/register"type="button" class="text-black bg-[#3FC43B] hover:bg-[#53B950] font-medium rounded-lg text-sm px-4 py-2 text-center text-sm px-5 py-2.5 text-center me-2 mb-2">Daftar</a>
      <?php endif;?>  
    ...
    <div class="items-center justify-between hidden w-full md:flex md:w-auto md:order-1" id="navbar-cta">
      <ul class="flex flex-col font-medium p-4 md:p-0 mt-4 border border-gray-100 rounded-lg-50 md:space-x-8 rtl:space-x-reverse md:flex-row md:mt-0 md:border-0  ">
        <li>
          <a href="/" class="block py-2 px-3 md:p-0 text-white bg-[#3FC43B] rounded md:bg-transparent md:text-[#3FC43B]" aria-current="page">Top-up</a>
        </li>
        <li>
          <a href="/transaksi/invoice " class="block py-2 px-3 md:p-0 text-white rounded hover:bg-[#3FC43B] md:hover:bg-transparent md:hover:text-[#3FC43B]">Transaksi</a>
        </li>
        <li>
